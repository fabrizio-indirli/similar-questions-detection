# This model is adapted from https://github.com/aerdem4/kaggle-quora-dup
import pandas as pd 
import numpy as np
import re
...
def get_model(embedding_matrix, nb_words, n_features):    
    ...
    sequence_1_input = Input(shape=(max_sequence,), dtype="int32")
    embedded_sequences_1 = embedding_layer(sequence_1_input)
    x1 = lstm_layer(embedded_sequences_1)

    sequence_2_input = Input(shape=(max_sequence,), dtype="int32")
...
def preprocess(q):
    ...
    for word in q.split()[::-1]:
        if word in top_words:
            ...
            new_memento = True
        if len(processed_q) == max_sequence:
            break
...
features_train = np.hstack((train_nlp_features, train_graph_features, train_features_df))
...
train_q_a_padded = pad_sequences(tokenizer.texts_to_sequences(train_q_a), maxlen=max_sequence)
train_q_b_padded = pad_sequences(tokenizer.texts_to_sequences(train_q_a), maxlen=max_sequence)
...
features_test = np.hstack((test_nlp_features, test_graph_features, test_features_df))

test_q_a_padded = pad_sequences(tokenizer.texts_to_sequences(test_q_a), maxlen=max_sequence)
test_q_b_padded = pad_sequences(tokenizer.texts_to_sequences(test_q_a), maxlen=max_sequence)
